...
export const SlaveforwardBotComposer = new Composer<MyContext>();
...
SlaveforwardBotComposer.chatType(["private"]).command(
  "startforward",
  async (ctx, next) => {
    async function longtask() {
      try {
          ...
          if (!user?.isEngaged) {
            ...
            if (ctx.from.id == user_id) {
              ...
              if (text) {
                ...
                await ctx.reply(
                  "Reply to the message in the suggested format."
                );
              }
            }
          } else {
            ctx.reply(
              "A forward task is already running, you can't create another task unitil its completed. stop it using /stopforward"
            );
          }
        } else {
        ...
        if (
          ...
          ctx.reply(
            `there are no messages to forward, Maybe the messages are deleted`
          );
        }
      }
    }
...
SlaveforwardBotComposer.chatType(["private"]).command("start", async (ctx) => {
  try {
    ...
    await ctx.reply(
      `Hi, <b>I am a powerful forward bot,</b> created by <a href=\"https://t.me/Forward_father_bot\">@Forward_father_bot</a>. You can simply send me a text in the following format:
