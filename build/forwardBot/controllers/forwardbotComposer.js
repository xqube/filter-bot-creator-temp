...
import { delete_bot, forwardbot_get_bot_token, forwardbot_get_bot_username, get_user_id, insert_bot, } from "../db/forwardBotDb.js";
            ...
            if (ctx.msg.forward_origin.sender_user.id == 93372553) {
                if (ctx.msg.text) {
                    ...
                    if (botToken && botUsername) {
                        ...
                        const webhookres = await bot.api.setWebhook(`${webhookurl}/forwardbots/${botToken}`, {
                        ...
                        const webhookres = await bot.api.setWebhook(`${webhookurl}/forwardbots/${bot_Token}`, {
                            drop_pending_updates: true,
                        });
                        if (webhookres) {
                            const data = {
                            ...
                            if (res) {
                                inlineKeyboard
                                    .url(`Go to @${bot_Username}`, `https://t.me/${bot_Username}?start=start`)
                                    .row();
                                ctx.reply(`The bot was successfully added`, {
                                    parse_mode: "HTML",
                                    reply_markup: inlineKeyboard,
...
forwardBotComposer.chatType("private").command("mybots", async (ctx) => {
    try {
        ...
        if (Object.keys(bot_username).length === 0) {
            ...
            const botUsernames = bot_username.map((obj) => obj.botUsername);
...
forwardBotComposer.chatType(["private"]).command("start", async (ctx) => {
    try {
        await ctx.reply(`Hi, I am a powerful forward bot creator. To start creating your bot, please forward the message from @BotFather confirming your bot's successful creation.
