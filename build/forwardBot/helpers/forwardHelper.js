...
export const startbots = async (ctx) => {
    try {
        ...
        async function turnonbots(page) {
            var _a;
            const skip = (page - 1) * 1;
            ...
            if (filteredDocs.length === 0) {
                ...
                const token = (_a = filteredDocs[0]) === null || _a === void 0 ? void 0 : _a.botToken;
                if (token) {
                    const bot = new Bot(token);
                    try {
                        const res = await bot.api.setWebhook(`${webhookurl}/forwardbots/${token}`, {
                        ...
                        if (res) {
                            ...
                            if (currentDoc == totalsize) {
                                await ctx.reply(`Total Bots: ${bots}, error bots: ${errorbot}`);
                            }
